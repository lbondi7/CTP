...
private:
	...
	void createImageViews() {
		...
		for (uint32_t i = 0; i < swapChainData.swapChainImages.size(); i++) {
			swapChainData.swapChainImageViews[i] = createImageView(swapChainData.swapChainImages[i], swapChainData.swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
		}
	}
	...
	void createRenderPass() {
		...
		if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &graphicsData.renderPass) != VK_SUCCESS) {
	...
	void createCommandPool() {
		QueueFamilyIndices queueFamilyIndices = VkSetup::findQueueFamilies(physicalDevice, surface);
	...
	void createDescriptorSets() {
		...
		descriptorSets.resize(swapChainData.swapChainImages.size());
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(time * glm::radians(90.0f), 1.0f, time * glm::radians(90.0f)));
		ubo.view = glm::lookAt(glm::vec3(5.0f, 3.0f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		ubo.proj = glm::perspective(glm::radians(45.0f), swapChainData.swapChainExtent.width / (float)swapChainData.swapChainExtent.height, 0.1f, 100.0f);
		ubo.proj[1][1] *= -1;
